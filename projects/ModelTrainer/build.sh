...
# Change to ModelTrainer directory (script directory)
cd "$(dirname "$0")" || exit 1
...
if [ ! -d "Data" ] || [ -z "$(find Data -name 'intent_dataset_v*.json' -print -quit 2>/dev/null)" ]; then
    echo "Warning: No training dataset found in Data/ directory"
    echo "To get training data, run:"
